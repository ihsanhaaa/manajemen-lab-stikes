                                    ...
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            ...
                                            <form action="{{ route('konfirmasi-pembayaran.store') }}" method="POST" enctype="multipart/form-data">
                                                @csrf
                                                <div class="mb-3">
                                                    <label for="buktiPembayaran" class="form-label">Semester <span style="color: red">*</span> </label>
                                                    <input type="text"
                                                        class="form-control @error('buktiPembayaran') is-invalid @enderror" id="buktiPembayaran"
                                                        name="buktiPembayaran" value="{{ old('buktiPembayaran') }}" placeholder="contoh: Ganjil" required>
                                    ...
                                    <table id="datatable" class="table table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                                        ...
                                        <tbody>
                                                @foreach($buktiPembayarans as $key => $buktiPembayaran)
                                                    ...
                                                    <tr>
                                                        <th scope="row">{{ ++$key }}</th>
                                                        <td>{{ $buktiPembayaran->tanggal_bayar }}</td>
                                                        <td>{{ $buktiPembayaran->path_bukti_bayar }}</td>
                                                        <td>
                                                            @if(!$buktiPembayaran->is_active)
                                                                <form action="{{ route('buktiPembayarans.activate', $buktiPembayaran->id) }}" method="POST">
                                                                    @csrf
                                                                    <button type="submit" class="btn btn-success btn-sm" title="Aktifkan"><i class="fas fa-check"></i></button>
                                                                </form>
                                                            @endif
                                                            @if($buktiPembayaran->is_active)
                                                                <form action="{{ route('buktiPembayarans.deactivate', $buktiPembayaran->id) }}" method="POST">
                                                                    @csrf
